...
names(FavoriteSong) <- gsub(x = names(FavoriteSong), pattern = "\\.", replacement = " ")
colnames(FavoriteSong)[colnames(FavoriteSong)=="What was your pseudonym "] <- "pseudonym"
```
...
```{r}
Rating <- gather(FavoriteSong, "artists", "rating", 3:45)
Rating <- subset(Rating, select = -c(Timestamp))
```

```{r}
library("readr")
parse_datetime(Person$Timestamp, format="%m%d%%y %H:%M%S")
```